...
# ╔═╡ 700cc610-4963-11ef-0722-2be29a68cdb0
begin
	using PlutoUI, Plots, DifferentialEquations
end

# ╔═╡ 9d843d36-e81f-4e23-9c80-2f62c8538fe1
md"""
# Problema do Carvão - Fogler 🔥
"""

# ╔═╡ 79b86f4b-28b1-4281-b603-df7c94c0ac31
md"""
## Definindo os Parâmetros do Problema 🏭
"""

# ╔═╡ ed1acb80-7bfa-4a57-ae5e-fcb98e535233
begin
	slide1 = @bind v Slider(1:20, default= 10, show_value=true)
	num1 = @bind k1 NumberField(0.001:0.001:2, default= 0.12)
	num2 = @bind k2 NumberField(0.001:0.001:2, default= 0.046)
	num3 = @bind k3 NumberField(0.001:0.001:2, default= 0.02)
	num4 = @bind k4 NumberField(0.001:0.001:2, default= 0.034)
	num5 = @bind k5 NumberField(0.001:0.001:2, default= 0.04)

	con1 = @bind carvao NumberField(0:0.001:10, default= 2)
	con2 = @bind pre NumberField(0:0.001:10, default= 0)
	con3 = @bind asfalteno NumberField(0:0.001:10, default= 0)
	con4 = @bind oleo NumberField(0:0.001:10, default= 0)

	md"""
	Vazão de Entrada: $(slide1) dm³/min\
	K1 = $(num1)\
	K2 = $(num2)\
	K3 = $(num3)\
	K4 = $(num4)\
...
# ╔═╡ 25235cdd-2065-4ad4-a52a-a5c3b1418d4f
md"""
## Geração do Gráfico Interativo 📈
"""

# ╔═╡ 30e039ff-0ec7-4e0d-a05e-220d5faa9a01
md"""
### Códigos Utilizado
"""

# ╔═╡ bc8bea1f-4bbf-47ef-b1ae-8b67c6c7f7e9
md"""
Arthur Vilela - 24/07/2024
"""

# ╔═╡ 78d212c7-4b6e-42ac-9059-e2361b3d1ec4
function dsdt(derivadas, c, p, t)
	#c = [Cc, Cp, Ca, Co]
	derivadas[1] = p[6]*(-p[1]*c[1]-p[2]*c[1])
    derivadas[2] = p[6]*(p[1]*c[1]-p[4]*c[2]+p[3]*c[3])
    derivadas[3] = p[6]*(p[2]*c[1]+p[4]*c[2]-p[5]*c[3]-p[3]*c[3])
    derivadas[4] = p[6]*(p[5]*c[3])
end

# ╔═╡ ea8af673-09ac-43ea-8f22-55cad260ab35
p = [k1, k2, k3, k4, k5, v]

# ╔═╡ 53cfad03-96ed-494c-b925-231279dbd7a6
tspan = (0,30)

# ╔═╡ 70b034ca-8272-4ada-8062-b420ea664070
c_0 = [carvao, pre, asfalteno, oleo]

# ╔═╡ 240e0dec-6388-407e-95bd-aaacaa458a30
sistema = ODEProblem(dsdt, c_0, tspan, p)

# ╔═╡ 912fa570-0c04-4e3c-b178-996013f0adba
solucao = solve(sistema)

# ╔═╡ 44dfd723-142a-4601-a03f-2ac378c9b79d
plot(solucao, label = ["Cc" "Cp" "Ca" "Co"], 
	xlabel = "Tempo Espacial (τ)",
	ylabel = "Concentração (mol/dm³)", layout = (1))

# ╔═╡ c64c248e-5a71-4fd9-aa18-a6ac0ee3e32b
TableOfContents()
...
manifest_format = "2.0"
project_hash = "001ff9924a5b4b8032ea322f90995186e6eb1309"
